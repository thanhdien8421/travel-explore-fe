...
export default function HeroSection() {
    ...
    return (
        <section className="relative min-h-[60vh] w-full shadow-xl border border-gray-200 bg-gradient-to-br from-white via-gray-50 to-gray-100">
            <div className="grid grid-cols-1 md:grid-cols-2 min-h-[400px] md:min-h-[500px] w-full">
                {/* Left: Text */}
                <div className="flex flex-col justify-center px-4 sm:px-6 md:px-16 py-8 sm:py-16">
                    <h1
                    ...
                    <p className="text-sm sm:text-base md:text-lg lg:text-xl text-gray-800 mb-4 sm:mb-8">
                        Từ những địa điểm nổi tiếng đến những điểm dừng chân độc đáo, chúng tôi kết nối bạn với những điều tuyệt vời nhất ở Việt Nam.
                    </p>
                    <Link
                        href="#featured-locations"
                        className="inline-flex max-w-60 items-center px-4 sm:px-8 py-2 sm:py-4 bg-gradient-to-r from-blue-600 to-blue-700 text-white 
        font-semibold rounded-full hover:from-blue-700 hover:to-blue-700 transition-all duration-300 transform hover:scale-105 shadow-md text-sm sm:text-base"
                    >
                        <span>Khám phá ngay</span>
                ...
                <div className="relative w-full h-[250px] sm:h-[300px] md:h-[400px] lg:h-[500px] flex items-end">
                    {/* Get cover image - prioritize images array */}
                    {(() => {
                        let imageUrl = currentPlace.cover_image_url;
                        if (currentPlace.images && currentPlace.images.length > 0) {
                            const coverImage = currentPlace.images.find(img => img.is_cover);
                            if (coverImage) {
                                imageUrl = getImageUrl(coverImage.image_url);
                            } else if (currentPlace.images[0]) {
                                imageUrl = getImageUrl(currentPlace.images[0].image_url);
                            }
                        }
                        return (
                            <Image
