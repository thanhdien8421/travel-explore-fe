  ...
  const SidebarContent = () => (
    <>
      ...
      <nav className="flex-1 p-2 space-y-1 overflow-hidden">
        {navItems.map((item) => {
          let isActive = false;
          if (item.href === "/partner/dashboard") {
            isActive = pathname === item.href;
          } else if (item.href === "/partner/locations/add") {
            isActive = pathname === item.href;
          } else if (item.href === "/partner/locations") {
            isActive = pathname === item.href || (pathname.startsWith(item.href) && !pathname.includes("/add"));
          } else {
  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden fixed bottom-4 left-4 z-50 p-3 bg-blue-800 text-white rounded-full shadow-lg hover:bg-blue-700 transition-colors"
      >
        ...
        >
          ...
          >
            ...
            <div className="p-4 flex justify-between items-center border-b border-gray-100">
              <span className="font-semibold text-gray-900">Menu Đối tác</span>
              <button
                onClick={() => setIsMobileOpen(false)}
