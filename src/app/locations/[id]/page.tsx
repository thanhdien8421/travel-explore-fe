...
import AddToPlanModal from "@/components/travel-plan/add-to-plan-modal";
...
import { hasRating, formatRating } from "@/lib/rating-utils";
...
export default function LocationDetail({ params }: Props) {
  ...
  const markAsVisited = async () => {
    ...
    try {
      ...
      if (location.visited) {
        // Unmark as visited
        await apiService.removeVisit(location.id, token);
        setLocation({ ...location, visited: false });
      } else {
        // Mark as visited
            ...
            <div className="flex items-center gap-3">
              ...
              {isAuthenticated && (
                <button 
                  ...
                  title={location?.visited ? "Bỏ ghi nhận ghé thăm" : "Ghi nhận địa điểm đã ghé thăm"}
              ...
              {isAuthenticated && (
                <button 
                  ...
                  title="Lưu vào kế hoạch du lịch"
                  className="p-2 rounded-lg text-gray-600 hover:text-gray-900 hover:bg-gray-100 transition-colors">
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {/* AI Summary Section - Generated from reviews */}
            {location.summary && (
              <section className="bg-white rounded-lg shadow-sm p-6 border border-purple-100">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-purple-600 flex items-center gap-1">
                      Tổng hợp bởi AI từ đánh giá của người dùng
                    </p>
                  </div>
                </div>
            ...
            <section className="bg-white rounded-lg shadow-sm p-6">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2 bg-yellow-200/50 px-3 py-1 rounded-full">
                    <svg className="w-5 h-5 fill-yellow-400" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                    <span className="font-semibold text-yellow-800">{formatRating(location.average_rating)}</span>
                  </div>
                </div>
