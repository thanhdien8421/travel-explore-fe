...
export default function ContributorContributionsPage() {
  const router = useRouter();
  const [places, setPlaces] = useState<ContributorPlace[]>([]);
  ...
  const fetchContributions = async () => {
    try {
      setLoading(true);
      const response = await apiService.getContributorContributions({
        page,
        limit: 10,
      ...
      setError("Không thể tải danh sách đóng góp.");
      console.error("Error fetching contributor contributions:", err);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6 lg:mb-8">
          ...
          <Link
            ...
            className="px-4 sm:px-6 py-2 sm:py-3 bg-green-800 text-white rounded-xl font-semibold hover:bg-green-900 transition-colors flex items-center justify-center gap-2 text-sm sm:text-base"
          >
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6 items-center">
          <div className="flex-1">
            <input
              ...
              className="w-full px-4 py-2.5 border border-gray-200 rounded-xl bg-white focus:ring-1 focus:ring-green-600 focus:border-green-600 outline-none transition-all duration-200"
            />
          </div>
          <CustomDropdown
            value={statusFilter}
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Chưa có đóng góp nào</h3>
            <p className="text-gray-500 mb-6">Bắt đầu bằng việc đóng góp địa điểm đầu tiên của bạn</p>
            <Link
              href="/contributor/locations/add"
              className="inline-flex items-center gap-2 px-6 py-3 bg-green-800 text-white rounded-xl font-semibold hover:bg-green-900 transition-colors"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
              </svg>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex justify-end gap-2">
                          <Link
                            ...
                            title="Xem"
                            className="w-9 h-9 flex items-center justify-center border border-green-800 text-green-800 bg-white rounded-lg hover:bg-green-50 transition-colors"
                          >
                          ...
                          {(place.status === "PENDING" || place.status === "REJECTED") && (
                            <button
                              onClick={() => router.push(`/contributor/locations/${place.id}/edit`)}
                              title="Sửa"
